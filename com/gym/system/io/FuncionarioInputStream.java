package com.gym.system.io;

import com.fasterxml.jackson.databind.DeserializationFeature;
import com.fasterxml.jackson.databind.MapperFeature;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.fasterxml.jackson.databind.SerializationFeature;
import com.fasterxml.jackson.datatype.jsr310.JavaTimeModule;
import com.gym.system.models.Funcionario;
import com.gym.system.models.enums.Turno;

import java.io.IOException;
import java.io.InputStream;
import java.nio.charset.StandardCharsets;
import java.time.LocalDate;
import java.util.Scanner;

public class FuncionarioInputStream extends InputStream {
    private Funcionario[] funcionarios;
    private InputStream in;

    private static final ObjectMapper mapper = new ObjectMapper()
            .registerModule(new JavaTimeModule())
            .disable(SerializationFeature.WRITE_DATES_AS_TIMESTAMPS)
            .configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false)
            .configure(MapperFeature.ACCEPT_CASE_INSENSITIVE_ENUMS, true);

    public FuncionarioInputStream(InputStream in) {
        this.in = in;
    }

    public Funcionario[] readSystem() {
        Scanner sc = new Scanner(in);
        System.out.println("Quantidade de Funcionários:");
        int qnt = Integer.parseInt(sc.nextLine());

        this.funcionarios = new Funcionario[qnt];

        for (int i = 0; i < qnt; i++) {
            System.out.println("Informe o CPF do Funcionário:");
            String cpf = sc.nextLine();

            System.out.println("Informe o Nome do Funcionário:");
            String nome = sc.nextLine();

            System.out.println("Informe a Data de Nascimento do Funcionário (yyyy-MM-dd) ou deixe vazio:");
            String dataStr = sc.nextLine();
            LocalDate dataNascimento = dataStr.isBlank() ? null : LocalDate.parse(dataStr);

            System.out.println("Informe o Telefone do Funcionário:");
            String telefone = sc.nextLine();

            System.out.println("Informe o E-mail do Funcionário:");
            String email = sc.nextLine();

            System.out.println("Informe a Senha do Funcionário:");
            String senha = sc.nextLine();

            System.out.println("Informe o Salário do Funcionário:");
            float salario = Float.parseFloat(sc.nextLine());

            System.out.println("Informe o Turno do Funcionário (manha, tarde ou noite):");
            Turno turno = Turno.valueOf(sc.nextLine().toUpperCase());

            funcionarios[i] = new Funcionario(cpf, nome, dataNascimento, telefone, email, null, senha, salario, turno);
        }

        return funcionarios;
    }

    private Funcionario[] readJSON(InputStream source) throws IOException {
        String json = new String(source.readAllBytes(), StandardCharsets.UTF_8);
        return mapper.readValue(json, Funcionario[].class);
    }

    public Funcionario[] readFile() throws IOException {
        return readJSON(in);
    }

    public Funcionario[] readTCP() throws IOException {
        return readJSON(in);
    }

    @Override
    public int read() throws IOException {
        return (this.in != null) ? this.in.read() : -1;
    }
}